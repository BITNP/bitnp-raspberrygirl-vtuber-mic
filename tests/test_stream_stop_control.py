...
async def _flush_epoch_proof() -> None:
    ...
    registration = SourceRegistration("stream-001", RtpEndpoint("127.0.0.1", RtpPort(5004)))
...
async def _malformed_stop_proof() -> None:
    ...
    registration = SourceRegistration("stream-001", RtpEndpoint("127.0.0.1", RtpPort(5004)))
    control = WebSocketStreamingControl(
        _Connection(messages=[_stop(stream_id="stream-001", epoch=4).replace('"orchestrator"', '"sound"')]),
        ControlContext("trace-001", "session-001"),
    )
