...
DEFAULT_HOST_BYTEORDER: Final[HostByteOrder] = "little" if sys.byteorder == "little" else "big"
...
class PortAudioCaptureSource:
    """Capture one PCM16 frame through PortAudio and send it across the media boundary."""
    ...
    def capture_and_send(self, boundary: AudioFrameBoundary, sink: AudioRtpSink) -> AudioFrame | None:
...
class PortAudioBlockCapture:
    ...
    async def read_block(self) -> bytes | None:
        ...
        payload, _overflowed = await asyncio.to_thread(stream.read, PCM16_MONO_20MS_FRAME_SAMPLES)
        canonical_payload = _normalize_pcm16le(payload, self._byteorder)
        if len(canonical_payload) != PCM16_MONO_20MS_FRAME_BYTES:
            return None
