...
class WebSocketStreamingControl:
    ...
    async def wait_stop(self, registration: SourceRegistration) -> int:
        while True:
            raw_event = await self._connection.recv()
            if not isinstance(raw_event, str):
                raise ConfigError(key=SOURCE_STOP_EVENT, reason="must be a text control event")
            try:
                event = json.loads(raw_event)
            except json.JSONDecodeError as error:
                raise ConfigError(
            ...
            if isinstance(event, dict) and event.get("event_type") == SOUND_FLUSH_EVENT:
                continue
            if not isinstance(event, dict) or event.get("event_type") != SOURCE_STOP_EVENT:
                raise ConfigError(key=SOURCE_STOP_EVENT, reason="must be received after source readiness")
            if event.get("source") != "orchestrator" or event.get("session_id") != self._context.session_id:
                raise ConfigError(key=SOURCE_STOP_EVENT, reason="must be authenticated for this session")
            data = event.get("data")
            if not isinstance(data, dict) or data.get("stream_id") != registration.stream_id:
                raise ConfigError(key=SOURCE_STOP_EVENT, reason="must target the registered stream")
            epoch = data.get("cancellation_epoch")
            if type(epoch) is not int or epoch < 0:
                raise ConfigError(key=SOURCE_STOP_EVENT, reason="must contain a nonnegative cancellation epoch")
            previous_epoch = self._highest_stop_epochs.get(registration.stream_id)
            if previous_epoch is not None and epoch <= previous_epoch:
                raise ConfigError(key=SOURCE_STOP_EVENT, reason="must contain a newer cancellation epoch")
