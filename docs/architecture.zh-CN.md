...
媒体契约是每个完整的 16 kHz 单声道采集块对应一个固定的 RTP V2/PT96/L16 数据包，包含 320 个采样、20 ms 时长和 640 字节负载。`mic-capture` 使用 `sounddevice` 和 PortAudio，仅作为本地单帧标准输出诊断工具。Mic 不感知业务策略，因此产品场景和 Orchestrator 交互选择不会改变其契约。
