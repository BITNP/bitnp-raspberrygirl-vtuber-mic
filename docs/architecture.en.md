...
The media contract is one fixed RTP V2/PT96/L16 packet for each complete 16 kHz mono capture block: 320 samples, 20 ms, and 640 payload bytes. `mic-capture` uses `sounddevice` with PortAudio only as a local one-frame stdout diagnostic. Mic is mode agnostic, so `lecturer`, `virtual_streamer`, and `onsite_explainer` do not change its contract.
