...
- 可选本地 ZipEnhancer 降噪和 Silero VAD。VAD 在采集期间在线运行，静音后形成端点；ZipEnhancer 在端点窗口上降噪后再供 CAM++ 与 ASR 使用。
