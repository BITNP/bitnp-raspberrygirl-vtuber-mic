...
部署时根据 `.env.example` 配置 Orchestrator WSS control URL、`TRUSTED_LAN_TOKEN`、`ORCHESTRATOR_TLS_CA_PATH`、Mic ASR endpoint/model、Orchestrator RTP ingress、本地 UDP bind、stream ID、timestamp、trace ID 和 session ID。`ORCHESTRATOR_TLS_CA_PATH` 指向只读 PEM CA bundle，用于校验 Orchestrator WSS；Mic 的 ASR HTTPS 信任配置使用部署环境的系统信任库。生产环境必须使用 `wss://` 和 token；`ws://` 只允许在显式设置 `MIC_ALLOW_LOOPBACK_WS=true` 的回环测试中使用。
